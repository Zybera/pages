...
window.onload = function () {
  ...
  function documentActions(e) {
    ...
    if (window.innerWidth > 768 && detectMob()) {
      ...
      if (!targetElement.closest('.menu__item') && document.querySelectorAll('.menu__item._hover').length > 0) {
        ...
        for (let i = 0; i < hoverItems.length; i++) {
          hoverItems[i].classList.remove('_hover');
        }
      }
    }
    ...
    if (targetElement.classList.contains('cart-list__delete')) {
      e.preventDefault();
      const productId = targetElement.dataset.cartid;
  ...
  function updateCart(productButton, productId, productAdd = true) {
    ...
    const cartProduct = document.querySelector(`[data-cartid="${productId}"]`);
    ...
    if (productAdd) {
      ...
      if (!cartProduct) {
        ...
        const cartProductContent = `
          ...
          <div class="cart-list__body">
            ...
            <a href="" data-cartid="${productId}" class="cart-list__delete">Delete</a>
          </div>
        `;
        cartList.insertAdjacentHTML('beforeend', `<li data-cartid="${productId}" calss="cart-list__item">${cartProductContent}</li>`);
      } else {
      ...
      const cartProductQuantuty = cartProduct.querySelector('.cart-list__quantity span');
      cartProductQuantuty.innerHTML = --cartProductQuantuty.innerHTML;
      if (!parseInt(cartProductQuantuty.innerHTML)) {
        cartProduct.remove();
      }
